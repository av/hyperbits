...
import { simulateParticles } from "../particles/simulator";
import type { SpawnerConfig, BehaviorConfig } from "../particles/types";
...
describe("simulateParticles", () => {
  describe("max limit enforcement", () => {
    ...
    it("should enforce max limit per spawner independently", () => {
      ...
      const spawner1Particles = result.filter(
        (p) => p.spawnerId === "spawner-1",
      );
      const spawner2Particles = result.filter(
        (p) => p.spawnerId === "spawner-2",
      );
  ...
  describe("per-spawner startFrame", () => {
    ...
    it("should work with burst spawning", () => {
      ...
      const spawner1Particles = resultFrame0.filter(
        (p) => p.spawnerId === "spawner-burst-no-offset",
      );
      const spawner2Particles = resultFrame0.filter(
        (p) => p.spawnerId === "spawner-burst-offset",
      );
